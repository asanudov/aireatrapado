...
# LÓGICA DE INTERFAZ CONTROLADA POR PYTHON (Botón único de reset):
...
# B. Parámetros Hidráulicos Estacionarios (Campos de texto limpios)
...
# C. NUEVOS PARÁMETROS CRITERIO ANTI-COLAPSO (Wang et al., 2023 - AQUA IWA)
st.sidebar.markdown("**Protección Anticolapso por Vacío (IWA)**")
...
# Conversión segura de variables
try:
    ...
    st.sidebar.error("Por favor, introduce valores numéricos válidos en los campos.")
    q_m3s, d_m, dh_d, h_res = 0.075, 0.305, 9.0, 0.0
...
if uploaded_file:
    try:
        ...
        if col_x and col_z:
            ...
            # Condición crítica de arrastre insuficiente (tramo descendente S > 0)
            ...
            # ---- MOTOR DE CÁLCULO 2: CRITERIO DE PENDIENTES LARGAS ANTI-COLAPSO (WANG ET AL., 2023) ----
            # Evaluamos la caída/subida vertical acumulada desde el último punto de control (origen o cresta previa)
            df['critico_pendiente_larga'] = False
            ultimo_z_control = df.loc[0, col_z]
            
            for i in range(1, len(df)):
                z_actual = df.loc[i, col_z]
                diff_vertical = abs(ultimo_z_control - z_actual)
                
                # Si la diferencia vertical acumulada rompe el criterio de la ecuación universal de vacío:
                if diff_vertical > limite_critico_vertical:
                    df.loc[i, 'critico_pendiente_larga'] = True
                    ultimo_z_control = z_actual  # Se asume una válvula de aire intermedia propuesta aquí
                
                # Las crestas geométricas naturales actúan inherentemente como nodos de control
                if df.loc[i, 'es_cresta']:
                    ultimo_z_control = z_actual
            ...
            # Hito 1: Puntos Altos Geométricos (Acumulación clásica por flotación)
            ...
            # NUEVO HITO 3: Puntos de colapso potencial en pendientes largas (Wang et al., 2023)
            ...
            fig.add_trace(go.Scatter(
                ...
                name='Recomendación Válvula Intermedia (Criterio Vacío IWA)'
            ))
            ...
            # Filtramos filas que califiquen en cualquiera de los criterios de riesgo o diseño
            ...
            if not res_table.empty:
                ...
                elecciones = [
                    ...
                    f"Riesgo de Vacío por Rotura en Pendiente Larga (ΔZ > {limite_critico_vertical}m)",
                ...
                with c2:
                    st.metric(label="Límite Máximo de Caída Vertical sin Válvula (ΔH_D + h)", value=f"{limite_critico_vertical:.2f} m")
            else:
                st.success("✅ El sistema opera dentro de rangos óptimos. No se detectaron fallos cinemáticos ni riesgos estructurales por vacío.")
...
# 6. BIBLIOGRAFÍA DE RESPALDO INSTITUCIONAL (ACTUALIZADA)
st.markdown("""
